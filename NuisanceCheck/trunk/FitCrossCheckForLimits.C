...
void rebinVariableBins(TH1* histo,int bin){
	
	//2012
	vector<vector<float> > bin_vect;
	bin_vect.push_back(std::vector<float>());
	bin_vect.back().push_back(0); //0
	bin_vect.back().push_back(64); //1
	bin_vect.back().push_back(80); //2
	bin_vect.back().push_back(92); //3
	bin_vect.back().push_back(104); //4
	bin_vect.back().push_back(116); //5
	bin_vect.back().push_back(132); //6
	bin_vect.back().push_back(176); //7
	bin_vect.back().push_back(256); //8
	
	bin_vect.push_back(std::vector<float>());
	bin_vect.back().push_back(0); //0
	bin_vect.back().push_back(64); //0
	bin_vect.back().push_back(80); //0
	bin_vect.back().push_back(92); //0
  	bin_vect.back().push_back(104); //1
	bin_vect.back().push_back(116); //2
	bin_vect.back().push_back(132); //3
	bin_vect.back().push_back(152); //5
	bin_vect.back().push_back(176); //6
	bin_vect.back().push_back(256); //0
	...
	bin_vect.back().push_back(0); //0
	bin_vect.back().push_back(64); //1
	bin_vect.back().push_back(72); //2
	bin_vect.back().push_back(80); //3
	bin_vect.back().push_back(84); //4
	bin_vect.back().push_back(88); //5
	bin_vect.back().push_back(92); //6
	bin_vect.back().push_back(96); //7
	bin_vect.back().push_back(100); //8
	bin_vect.back().push_back(104); //9
	bin_vect.back().push_back(108); //10
	bin_vect.back().push_back(112); //11
	bin_vect.back().push_back(116); //12
	bin_vect.back().push_back(120); //13
	bin_vect.back().push_back(124); //14
	bin_vect.back().push_back(128); //15
	bin_vect.back().push_back(132); //16
//	bin_vect.back().push_back(136); //17
	bin_vect.back().push_back(140); //18
//	bin_vect.back().push_back(144); //19
	bin_vect.back().push_back(156); //20
//	bin_vect.back().push_back(160); //21
//	bin_vect.back().push_back(168); //22
	bin_vect.back().push_back(176); //23
//	bin_vect.back().push_back(184); //24
//	bin_vect.back().push_back(200); //25
	bin_vect.back().push_back(256); //26
	
	bin_vect.push_back(std::vector<float>());
	bin_vect.back().push_back(0); //0
	bin_vect.back().push_back(64); //1
	bin_vect.back().push_back(80); //2
	bin_vect.back().push_back(84); //3
	bin_vect.back().push_back(88); //4
	bin_vect.back().push_back(92); //5
	bin_vect.back().push_back(96); //6
	bin_vect.back().push_back(100); //7
	bin_vect.back().push_back(104); //8
	bin_vect.back().push_back(108); //9
	bin_vect.back().push_back(112); //10
	bin_vect.back().push_back(116); //11
	bin_vect.back().push_back(120); //12
	bin_vect.back().push_back(124); //13
	bin_vect.back().push_back(128); //14
	bin_vect.back().push_back(132); //15
	bin_vect.back().push_back(136); //16
	bin_vect.back().push_back(140); //17
	bin_vect.back().push_back(148); //18
	bin_vect.back().push_back(156); //19
	bin_vect.back().push_back(176); //20
//	bin_vect.back().push_back(200); //21
	bin_vect.back().push_back(256); //22
	
	std::vector<float> bins = bin_vect.at(bin);
...
namespace LimitCrossCheck{  
  ...
  bool UseMinosError(true);                // compute minos error (if false : use minuit error)
  ...
  TCanvas* DrawShift(TString channel, TString var, TString comp, double mu, TH1* d, TH1* n, TH1* p1s, TH1* m1s) {
    ...
    if(d) { cout << "N(Observed) = " << d->Integral() << endl; }
	  ...
	  var.ReplaceAll("ATLAS_","");
	  var.ReplaceAll("ANA_","");
	  var.ReplaceAll("HH_","");
	  var.ReplaceAll("MVAlike","");
	...
	gStyle->SetTitleYOffset(1.0);
	  ...
	  TCanvas *canvas = new TCanvas(cname,cname,800,600);
	  ...
	  n->GetYaxis()->SetTitleSize(0.06);
	  n->GetYaxis()->SetLabelSize(0.06);
	  
	  int bin = -1;
	  if(channel.Contains("vbf_tight_lowdr_2012")) bin = 0;
	  if(channel.Contains("vbf_tight_highdr_2012")) bin = 1;
	  if(channel.Contains("vbf_loose_highdr_2012")) bin = 2;
	  if(channel.Contains("boost_tight_2012")) bin = 3;
	  if(channel.Contains("boost_loose_2012")) bin = 4;
    
    // style
    if(d) {
		if(bin>-1)rebinVariableBins(d,bin);
      d->SetLineColor(1);
      d->SetLineWidth(1); 
      d->SetMarkerColor(1); 
      d->SetMarkerSize(0.9); 
	  ...
	  if(bin>-1)rebinVariableBins(n,bin);
	  if(bin>-1)rebinVariableBins(p1s,bin);
	  if(bin>-1)rebinVariableBins(m1s,bin);
    ...
    if(m1s_ratio->GetMinimum() < min) { min = m1s_ratio->GetMinimum(); }
	  if(max>14. || min<-14){max=14.; min=-14.;}
    p1s_ratio->SetMaximum( 1.5*max );
    p1s_ratio->SetMinimum( min - 0.5*fabs(min) );

	  
	  p1s_ratio->GetYaxis()->CenterTitle(true);
	  p1s_ratio->GetYaxis()->SetNdivisions(605,kTRUE);
	  p1s_ratio->GetYaxis()->SetLabelSize(t_size*height_1/height_2);
	  p1s_ratio->GetYaxis()->SetTitleSize(t_size*height_1/height_2);
	  p1s_ratio->GetYaxis()->SetTitleOffset(t_off*height_2/height_1);
	  
	  p1s_ratio->GetXaxis()->SetLabelSize(t_size*height_1/height_2);
	  p1s_ratio->GetXaxis()->SetTitleSize(t_size*height_1/height_2);
	  p1s_ratio->GetXaxis()->SetTitleOffset(1.2);
	  p1s_ratio->GetXaxis()->SetLabelOffset(t_off*0.05*height_2/height_1);
	  
    p1s_ratio->SetTitle("");
	p1s_ratio->GetXaxis()->SetTitle("m_{#tau#tau}^{MMC} [GeV]");
    p1s_ratio->GetYaxis()->SetTitle("Rel. unc. [%]");
    p1s_ratio->Draw("hist");
	  
	  TLine line(p1s_ratio->GetXaxis()->GetXmin(),0.,p1s_ratio->GetXaxis()->GetXmax(),0.);
	  line.SetLineColor(1);
	  line.SetLineStyle(2);
	  line.SetLineWidth(1);
	  line.DrawClone();
	p1s_ratio->Draw("hist same");
    m1s_ratio->Draw("hist same");
    if (d){
      TH1F *d_ratio = (TH1F*) d->Clone();
      d_ratio->Add(n,-1); d_ratio->Divide(n); d_ratio->Scale(100);
		d_ratio->SetMaximum(20); d_ratio->SetMinimum(-20);
      d_ratio->Draw("E1 same");
    }

    
    // write average shift on canvas
	  if (!plotRelative) 	  canvas->cd(1);
...
//    niceinfo->Draw("same");
	  ...
	  TString info("#bf{"+var+"}");
	  TLatex *niceinfo = new TLatex(0.18, 0.82, info);
	  niceinfo->SetNDC();
	  niceinfo->SetTextSize(0.06);
//    niceinfo->Draw("same");
	  if(comp=="VBFH") comp = "VBF H#rightarrow#tau#tau";
	  if(comp=="ggH") comp = "ggF H#rightarrow#tau#tau";
	  if(comp=="Ztt") comp = "Z#rightarrow#tau#tau";
	  if(comp=="QCD") comp = "Multi-jets";
	  info = "#bf{"+comp+"}";
	  niceinfo = new TLatex(0.18, 0.74, info);
	  niceinfo->SetNDC();
	  niceinfo->SetTextSize(0.06);
	  niceinfo->Draw("same");
	  if(channel.Contains("boost_tight_2012")) channel = "Boosted High-p_{T}^{#tau#tau} 2012";
	  if(channel.Contains("boost_loose_2012")) channel = "Boosted Low-p_{T}^{#tau#tau} 2012";
	  if(channel.Contains("vbf_tight_lowdr_2012")) channel = "VBF High-p_{T}^{#tau#tau} 2012";
	  if(channel.Contains("vbf_tight_highdr_2012")) channel = "VBF Low-p_{T}^{#tau#tau} Tight 2012";
	  if(channel.Contains("vbf_loose_highdr_2012")) channel = "VBF Low-p_{T}^{#tau#tau} Loose 2012";
	  info = "#bf{"+channel+"}";
	  niceinfo = new TLatex(0.18, 0.82, info);
	  niceinfo->SetNDC();
	  niceinfo->SetTextSize(0.06);
	  niceinfo->Draw("same");
  ...
  void Plot1DResponse(RooAbsReal* nll, RooRealVar* var, TString cname, TCanvas* can, TF1* poly, bool IsFloating, TLatex* latex, TDirectory* tdir, RooArgSet* SliceSet) {
    ...
    gROOT->cd();
    if(drawPlots) {
      TString epsDir(OutputDir+"/LHoodPlots/"+vname+"/");
