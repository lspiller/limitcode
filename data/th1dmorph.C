...
//       valid set of wt1,wt2 so why not take the average?
  ...
  if (idebug >= 1) cout << "Input histogram content sums: " 
			<< hist1->Integral() << " " << hist2->Integral() << endl;
// *         
...
//  Double_t *dist2=hist2->GetArray();
  Double_t *dist1 = new Double_t[1+nb1];
  Double_t *dist2 = new Double_t[1+nb1];
  for(Int_t i=1;i<nb2+1;i++) dist1[i]=hist1->GetBinContent(i);
  for(Int_t i=1;i<nb2+1;i++) dist2[i]=hist2->GetBinContent(i);
  ...
  if (idebug >= 1) {
    std::cout << "integral1 : " << hist1->Integral() << std::endl;
    std::cout << "integral2 : " << hist2->Integral() << std::endl;
  }
  if (idebug >= 3) {
