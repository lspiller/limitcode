...
void drawPlot_limit(string cardName, bool rereadAscii = 0, bool plotZoom = 0, string overlayCard = "", string overlayCard2 = "", string overlayCard3 = "") {
  ...
  if (dogg_4l) {
    ...
    t.DrawLatex(labelPosX + 0.4, labelPosY, "H#rightarrow#tau#tau");
	  ...
	  TLatex t;
	  t.SetNDC();
	  t.DrawLatex(labelPosX + 0.315, labelPosY+0.01, "#bf{H#rightarrow#tau_{jet}#tau_{jet}+2#nu}");
  }
  else if (docb) {
    minLimit = 0.05;
    maxLimit = 20;
...
void drawPlot_limit2(string cardName) {
  ...
  TLegend* leg = makeLeg();
  if (doObs) {
    leg->AddEntry(g_obs, (string("Observed ")+cardOpts).c_str(),"lp");
  }
  leg->AddEntry(g_exp, (string("Expected ")+cardOpts).c_str(),"l");
  leg->AddEntry(g_1s, "#pm1 #sigma-expected","f");
  leg->AddEntry(g_2s, "#pm2 #sigma-expected","f");
  
  if (overlay != "") {
    string suf = "VBF";
    applyOverlay(overlay, suf);
    if (overlayObs) {
      leg->AddEntry(g_ol_obs, (string("Observed ")+suf).c_str(), "lp");
    }
    if (overlayExp) {
      leg->AddEntry(g_ol_exp, (string("Expected ")+suf).c_str(), "l");
    }

    if (overlayBands) {
      if (overlayNegativeBands) {
        leg->AddEntry(g_ol_p2s, (string("#pm2#sigma-expected ")+suf).c_str(), "l");
        leg->AddEntry(g_ol_p1s, (string("#pm1#sigma-expected ")+suf).c_str(), "l");
      }
      else {
        leg->AddEntry(g_ol_p2s, (string("+2#sigma-expected ")+suf).c_str(), "l");
        leg->AddEntry(g_ol_p1s, (string("+1#sigma-expected ")+suf).c_str(), "l");
      }
    }
  ...
  if (overlay2 != "") {
    ...
    if (overlayObs) {
      leg->AddEntry(g_ol_obs2, (string("Observed ")+suf).c_str(), "lp");
    }
    if (overlayExp) {
      leg->AddEntry(g_ol_exp2, (string("Expected ")+suf).c_str(), "l");
    }

    if (overlayBands) {
      if (overlayNegativeBands) {
        leg->AddEntry(g_ol_p2s2, (string("#pm2#sigma-expected ")+suf).c_str(), "l");
        leg->AddEntry(g_ol_p1s2, (string("#pm1#sigma-expected ")+suf).c_str(), "l");
      }
      else {
        leg->AddEntry(g_ol_p2s2, (string("+2#sigma-expected ")+suf).c_str(), "l");
        leg->AddEntry(g_ol_p1s2, (string("+1#sigma-expected ")+suf).c_str(), "l");
      }
    }
  }
...
//    leg->AddEntry(g_ol_obs3, (string("Obs. ")+suf).c_str(), "lp");
    leg->AddEntry(g_ol_exp3, (string("Expected ")+suf).c_str(), "l");
  }
  leg->Draw();
