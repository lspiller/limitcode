...
void drawPlot_p0(string cardName, bool rereadAscii = 0, bool showZoom = 0, string overlayCard="", string overlayCard2="", string overlayCard3="") {
  ...
  if (dogg_4l_lvlv) {
    ...
    t.DrawLatex(labelPosX + 0.4, labelPosY, "H#rightarrow#tau#tau");
	  ...
	  TLatex t;
	  t.SetNDC();
	  t.DrawLatex(labelPosX + 0.335, labelPosY+0.01, "#bf{H#rightarrow#tau_{jet}#tau_{jet}+2#nu}");
  }
  else if (dovh) {
    lumi       = "4.7";
...
void drawPlot_p02(string cardName) {
  ...
  if (overlay != "") {
    ...
    g_ol_obs->SetLineColor(kBlue);
    g_ol_obs->SetMarkerColor(kBlue);
    g_ol_obs->SetLineStyle(1);
    g_ol_obs->SetLineWidth(3);
    ...
    if (overlayExp) {
      ...
      g_ol_exp->SetLineColor(kBlue);
      g_ol_exp->SetMarkerColor(kBlue);
      g_ol_exp->SetMinimum(minLimit);
      g_ol_exp->SetMaximum(maxLimit);
      g_ol_exp->SetLineStyle(2);
      g_ol_exp->SetLineWidth(3);
  ...
  if (overlay2 != "") {
    ...
    g_ol_obs2->SetLineColor(kViolet);
    g_ol_obs2->SetMarkerColor(kViolet);
    g_ol_obs2->SetLineStyle(1);
    g_ol_obs2->SetLineWidth(3);
    ...
    g_ol_exp2->SetLineColor(kViolet);
    g_ol_exp2->SetMarkerColor(kViolet);
    g_ol_exp2->SetLineStyle(2);
    g_ol_exp2->SetLineWidth(3);
    g_ol_exp2->Draw("l");
  }

  if (overlay3 != "") {
    TGraph* g_ol_obs3 = makeGraph(ytitle, nrOlPoints3, olPoints3, olObs3);
    g_ol_obs3->SetLineColor(kGreen+1);
    g_ol_obs3->SetMarkerColor(kGreen+1);
    g_ol_obs3->SetLineStyle(1);
    g_ol_obs3->SetLineWidth(3);
    ...
    g_ol_exp3->SetLineColor(kOrange+7);
    g_ol_exp3->SetMarkerColor(kOrange+7);
    g_ol_exp3->SetLineStyle(2);
    g_ol_exp3->SetLineWidth(3);
    g_ol_exp3->Draw("l");
  }
  ...
  if (uncap) {
    t.DrawLatex(lposx_hi+delta, (1-ROOT::Math::gaussian_cdf(0))*0.8, "#color[2]{#bf{0#sigma}}");
  }
  ...
  while (minLimit < 1-ROOT::Math::gaussian_cdf(n)) {
    ...
    stringstream str;
    str << "#color[2]{#bf{" << n << "#sigma}}";
    t.DrawLatex(lposx_hi+delta, (1-ROOT::Math::gaussian_cdf(n))*0.7, str.str().c_str());
    // cout << n << endl;
    n++;
  }
  c1->SetLogy(1);
