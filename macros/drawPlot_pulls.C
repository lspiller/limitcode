...
Color_t color_prefit_ol    = kBlue+2;
Color_t color_postfit      = kRed-4;
Color_t color_postfit_ol   = kRed+2;
...
string removeAlpha(string input){
	...
	while(input.find("TAU_")!=std::string::npos){
		pos = input.find("TAU_");
		input.erase(pos,4);
	}
	return input;
...
void drawPlot_pulls(string mass = "125", string cardName = "comb_MVAlike_2011_2012_pulls", bool remakeAscii = 1, string overlayCard="") {
    ...
    TPad *pad2 = new TPad("pad2", "pad2", 0.3, 0.1, 0.7, 0.22, 0);

    if (drawParamNames) pad1->SetLeftMargin(0.3);
    ...
    t3.SetNDC();
    t3.SetTextSize(0.03);
    string lumiLatex("#bf{#sqrt{s} = 7+8 TeV}");
    t3.DrawLatex(0.35,channelPosY-0.01,lumiLatex.c_str());
	lumiLatex = "#bf{#lower[-0.2]{#scale[0.6]{#int}}Ldt = 4.6-20.3 fb^{-1}}";
	t3.DrawLatex(0.35,channelPosY-0.040,lumiLatex.c_str());

    TLatex t2;
    t2.SetTextSize(0.03);
    t2.SetNDC();
    t2.DrawLatex(0.35, channelPosY-0.07, ("#bf{m_{H}="+mass+" GeV}").c_str());
...
void drawPlot_pulls2(string cardName, string mass, TCanvas* c1, TPad* pad1, TPad* pad2) {
    ...
    fileHolder nfs;
    drawPlot("ascii/"+cardName+"_pulls_nf.txt", 8, pulls);
    ...
    fileHolder nfs_ol;
    if (overlay != "") drawPlot("ascii/"+overlay+"_pulls_nf.txt", 8, pulls_ol);
    ...
    while (1) {
        if (!idFile2.good() || nlines > nrNuis-1) break;
        string nf_label;
        idFile2 >> nf_label;
        labels.push_back(nf_label);
        nlines++;
    }
    ...
    if (overlay != "") {
        ...
        while (1) {
            if (!idFile2_ol.good() || nlines_ol > nrNuis_ol-1) break;
            string nf_label;
            idFile2_ol >> nf_label;
			if(nf_label=="") break;
            labels_ol.push_back(nf_label);
            nlines_ol++;
        }
    }
    ...
    for (int i = 0; i < nrNuis; i++) {
		...
		if(index.find("stat")!=string::npos){
//			for(int j=0;j<nuis_map[index].size();j++){
				nuis_map[index].at(0) -=1.*scale_theta;
//			}
		}
    ...
    if (overlay != "") {
        for (int i = 0; i < nrNuis_ol; i++) {
			...
			if(index.find("stat")!=string::npos){
				//			for(int j=0;j<nuis_map[index].size();j++){
				nuis_map_ol[index].at(0) -=1.*scale_theta;
				//			}
			}
        }
    }
    ...
    std:: string latex_file_name("error.tex");
	...
	latex_file << "\\begin{table}\n";
	latex_file << "\\begin{center}\n";
	latex_file << "\\begin{tabular}{r|cc|cc}\n";
	latex_file << "\\hline \\hline \n";
	latex_file << "NP & post + & post - & var + & var - \\\\ \n";
	
	for (int i = nrNuis-1; i > -1; i--) {
		
		latex_file << labels[i]<< " & ";
		
		float tmp_poi_up = (poi_up[i]-poi_hat[i])/sigma_tot_hi;
		float tmp_poi_down = -(poi_down[i]-poi_hat[i])/sigma_tot_lo;

		float tmp_poi_nom_up = (poi_nom_up[i]-poi_hat[i])/sigma_tot_hi;
		float tmp_poi_nom_down = -(poi_nom_down[i]-poi_hat[i])/sigma_tot_lo;
		
		tmp_poi_nom_up = (tmp_poi_up-tmp_poi_nom_up)/tmp_poi_nom_up;
		tmp_poi_nom_down = (tmp_poi_down-tmp_poi_nom_down)/tmp_poi_nom_down;
			
		latex_file << tmp_poi_up << " & " << tmp_poi_down << " & " << tmp_poi_nom_up << " & " << tmp_poi_nom_down << " \\\\ \n";
	...
	TGraphAsymmErrors* gr_nf_ol;
    if (overlay != "") {
		gr_nf_ol = makeGraphErr("", nrNFs, getAry(nf_val_ol), getAry(points_nf_ol), getAry(nf_down_ol), getAry(nf_up_ol), NULL, NULL);
		gr_nf_ol->SetLineColor(1);
		gr_nf_ol->SetMarkerColor(1);
		gr_nf_ol->SetMarkerStyle(24);
		gr_nf_ol->SetLineStyle(2);
		gr_nf_ol->SetLineWidth(1);
		gr_nf_ol->SetMarkerSize(markerSize);
		gr_nf_ol->GetXaxis()->SetTitleOffset(1.2);
    }
    ...
    if (overlay != "") {
        ...
        gr_poi_nom_ol->SetFillStyle(3004);
    ...
    for (int i = offset; i < nrNuis+offset; i++){
		string my_label = drawParamNames?labels[i-offset]:"";
		my_label = removeAlpha(my_label);
		h->GetYaxis()->SetBinLabel(i+1, my_label.c_str());
	}
    h->LabelsOption("h");
    ...
    for (int i = 0; i < nrNFs; i++){
		string my_label = drawParamNames?nf_labels[i]:"";
		my_label = removeAlpha(my_label);
		h2->GetYaxis()->SetBinLabel(i+1,my_label.c_str());}
    h2->SetStats(0);
...
//    if (overlay != "") gr1s_ol->Draw("p2");
    ...
    TLegend* leg = makeLeg();
    leg->SetX1(channelPosX+0.42);
    leg->SetY1(channelPosY-0.0775);
    leg->SetX2(channelPosX + 0.67);
    leg->SetY2(channelPosY+0.01);
    leg->SetTextSize(0.0225);

//    leg->AddEntry(gr1s, "#pm1#sigma","f");
//    if (overlay != "") leg->AddEntry(gr1s_ol, "v+b #pm1#sigma","f");
    if (drawPrefitImpactBand) {
        leg->AddEntry(gr_poi_nom, "Prefit #Delta#hat{#mu}/#Delta#hat{#mu}_{tot}","f");
        if (overlay != "") leg->AddEntry(gr_poi_nom_ol, "v+b Prefit #Delta#hat{#mu}/#Delta#hat{#mu}_{tot}","f");
    }
    if (drawPostfitImpactBand) {
        leg->AddEntry(gr_poi, "Postfit #Delta#hat{#mu}/#Delta#hat{#mu}_{tot}","f");
        if (overlay != "") leg->AddEntry(gr_poi_ol, "v+b Postfit #Delta#hat{#mu}/#Delta#hat{#mu}_{tot}","f");
    }
    if (drawHatchedBands) {
...
void ROOT2Ascii(string folder) {
    ...
    TSystemDirectory  dire(("root_files/"+folder).c_str(), ("root_files/"+folder).c_str());
