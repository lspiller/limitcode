...
void globalInjection_TES(double mass,
        string folderIn,
		string folderOut,
        ...
        double target_mass = 0)
{
  ...
  TFile* file = new TFile(fileName(mass, folderIn).c_str());
...
//  }
  ...
  for (int imass=0;imass<nrPoints;imass++)
  {
    ...
    TFile f(fileName(thisMass, folderOut).c_str());
    cout<<fileName(thisMass, folderOut).c_str()<<endl;
