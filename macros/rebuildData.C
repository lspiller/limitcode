...
RooDataSet* rebuildData(RooWorkspace* this_w, RooWorkspace* w, RooDataSet* data, string name)
{
  ...
  while ((tt = (RooCatType*)catItr->Next()))
  {
	...
	continue;
      ...
      string ds_string(ds->GetName());
      string tt_string(tt->GetName());
      string ds_string_new = removeMass(ds_string);
      string tt_string_new = removeMass(tt_string);

      if (ds_string_new != tt_string_new && !isZvvHorZZ) continue;
...
string removeMass(string str){
	...
	massPoints.push_back("150");
 	int nrPoints = massPoints.size();
		...
		size_t found = str.find(massPoints.at(i));
