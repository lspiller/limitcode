...
void runBreakdown(string inFileName,
    ...
    string dataName        = "asimovData",
    string poiName         = "SigXsecOverSM",
	string folder          = "comb_MVAlike_2011_2012_pulls", //Change this to get the right folder name to make in root_files
    string xmlName         = "macros/muhat_breakdown.xml",
    ...
    string loglevel        = "DEBUG")
{
    ...
    system(("mkdir -vp root_files/" + config->folder + "_breakdown_" + technique).c_str());
    stringstream fileName;
    fileName << "root_files/" << config->folder << "_breakdown_" << technique << "/" << config->catecory2eval << ".root";
            ...
            while (systematic != 0) {
                ...
                while (attr_syst != 0) {
                    ...
                    if (breakdown) {
                        ...
                        runBreakdown(config->inFileName, config->wsName, config->modelConfigName, config->dataName, config->poiName, config->folder,"config/tmp_"+systName+".xml", config->technique, systName, config->precision, config->corrCutoff, config->loglevel);
                    }
