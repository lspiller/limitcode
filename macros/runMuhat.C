    ...
    string nominalSnapshot = "")
{
  ...
  if (dataName=="asimovData")
  {
    cout << "Asimov data doesn't exist! Please, allow me to build one for you..." << endl;
    makeAsimovData(mc, 1, ws, mc->GetPdf(), data, 1);
   // ws->Print();
    data = (RooDataSet*)ws->data("asimovData_1");
  }
  ...
  if (conditionalSnapshot != "" && dataName=="asimovData") ws->loadSnapshot(conditionalSnapshot.c_str());
  ...
  double nll_hat = nll->getVal();
	cout<<"nll_hat "<<nll_hat<<endl;
	
	std::fstream syst_file;
	std:: string syst_file_name("nll_hat.txt");
	syst_file.open (syst_file_name.c_str(), std::fstream::in | std::fstream::out | std::fstream::app);
	syst_file <<inFileName<<"\t"<<nll_hat<< "\n";
	syst_file.close();
  ...
  outFileName << "root_files/" << outFolder;
