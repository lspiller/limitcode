...
void runPulls(const char* inFileName,
    ...
    const char* dataName = "asimovData_1", // obsData for obs
    const char* folder = "NinaTest10_NomSysFlagOn_SS_NObjetuncert",
	bool stat_NP = false,
    ...
    string loglevel = "DEBUG")
{
    ...
    system(("mkdir -vp root_files/" + string(folder) + "_pulls").c_str());
...
//        if(!stat_NP) if (varName.find("gamma_stat") != string::npos) {
...
//        }
    ...
    for (int in = 0; in < nrNuis; in++) {
        ...
        fileName << "root_files/" << folder << "_pulls/" << nuipName << ".root";
